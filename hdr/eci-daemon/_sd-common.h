...
/* Note that strictly speaking __deprecated__ has been available before GCC 6.
 * However, starting with GCC 6 it also works on enum values, which we are
 * interested in. Since this is a developer-facing feature anyway (as opposed to
 * build engineer-facing), let's hence conditionalize this to gcc 6, given that
 * the developers are probably going to use something newer anyway. */
